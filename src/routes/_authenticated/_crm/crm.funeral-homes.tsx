...
function FuneralHomesPage() {
  ...
  const updateFn = useServerFn(updateFuneralHome);
  const statsFn = useServerFn(getFuneralHomeStats);
  ...
  const statsQueries = useQueries({
    queries: (homes ?? []).map((h: any) => ({
      queryKey: ["crm", "funeral-home-stats", orgId, h.id],
      queryFn: () => statsFn({ data: { id: h.id, organizationId: orgId } }),
      staleTime: 30_000,
    })),
  });
  const statsById: Record<string, { totalDecedents: number; activeDecedents: number }> = {};
  (homes ?? []).forEach((h: any, i: number) => {
    const d = statsQueries[i]?.data;
    if (d) statsById[h.id] = d;
  });
  ...
  const updateMut = useMutation({
    mutationFn: () => updateFn({ data: { id: editing.id, ...form } }),
    onSuccess: () => {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
                ...
                <CardContent className="space-y-2 pt-4">
                  ...
                  <div className="flex items-center gap-2 pt-1 text-xs text-muted-foreground">
                    <Badge variant="outline">
                      {stats ? stats.activeDecedents : "—"} active
                    </Badge>
                    <Badge variant="outline">
                      {stats ? stats.totalDecedents : "—"} total
                    </Badge>
                  </div>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && closeEdit()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit funeral home</DialogTitle>
